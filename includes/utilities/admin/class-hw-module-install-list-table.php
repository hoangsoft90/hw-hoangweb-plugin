 ...
 * @access private
 */
class HW_Module_Install_List_Table extends WP_List_Table {
    ...
    public function prepare_items() {
        ...
        $this->items = HW_TGM_Module_Activation::get_register_modules(array('core' => false));   //$this->get_modules_repositories()
		...
		$this->set_pagination_args( array(
			'total_items' => count($this->items),//$api->info['results'],
    ...
    public function no_items() {
		if ( isset( $this->error ) ) {
			...
			$message = __( 'No modules match your request.' );
		}
...
<div class="wp-filter">
	...
	<?php _install_search_form( isset( $views['plugin-install-search'] ) );
    ?>
</div>
<?php
	}
...
<?php
    ...
    public function display_rows() {
		...
		foreach ( (array) $this->items as $plugin ) {
            ...
            if(isset($plugin['desc'])) $plugin['description'] = $plugin['desc'];
            else $plugin['description']  ='';
            //author
            if(!isset($plugin['author'])) $plugin['author'] = 'Hoangweb';
			...
			if ( current_user_can( 'install_plugins' ) || current_user_can( 'update_plugins' ) ) {
                ...
                $status['url'] = '?page=hw_install_module&action=install-module&module='. $plugin['slug']. '&_wpnonce='. $nonce;    //by hoangweb
			...
			$details_link   = HW_ADMIN_URL.( '/module-install.php?tab=module-information&amp;module=' . $plugin['slug'] .
								'&amp;TB_iframe=true&amp;width=600&amp;height=550&_root='.base64_encode((ABSPATH)) );
			...
			} else if(!empty($plugin['icons']['default'])){
				$plugin_icon_url = $plugin['icons']['default'];
			}
            else $plugin_icon_url= HW_HOANGWEB_IMAGES. '/module-box.png';
		...
		?>
		<div class="plugin-card module-box module-repository plugin-card-<?php echo sanitize_html_class( $plugin['slug'] ); ?>">
