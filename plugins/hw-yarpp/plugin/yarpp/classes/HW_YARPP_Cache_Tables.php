...
class HW_YARPP_Cache_Tables extends HW_YARPP_Cache {
	...
	public function is_enabled() {
		...
		if (in_array($wpdb->prefix . HW_YARPP_TABLES_RELATED_TABLE,$tabledata) !== false)
			return true;
		else
			return false;
	}
	...
	public function setup() {
		...
		$wpdb->query(
            "CREATE TABLE IF NOT EXISTS `".$wpdb->prefix. HW_YARPP_TABLES_RELATED_TABLE."` (
	...
	public function upgrade($last_version) {
		...
		if ( $last_version && version_compare('3.2.1b4', $last_version) > 0 ) {
			...
			// are no duplicates.
			$this->flush();
			$wpdb->query('ALTER TABLE ' . $wpdb->prefix . HW_YARPP_TABLES_RELATED_TABLE .
			  ' DROP PRIMARY KEY ,' .
		...
		if ( $last_version && version_compare('3.6b1', $last_version) > 0 ) {
			...
			if ( defined('HW_YARPP_TABLES_KEYWORDS_TABLE') )
				$old_keywords_table = $wpdb->prefix . HW_YARPP_TABLES_KEYWORDS_TABLE;
			else
				$old_keywords_table = $wpdb->prefix . 'hw_yarpp_keyword_cache';
	...
	public function cache_status() {
		...
		return $wpdb->get_var("select (count(p.ID)-sum(c.ID IS NULL))/count(p.ID)
			...
			LEFT JOIN `{$wpdb->prefix}" . HW_YARPP_TABLES_RELATED_TABLE . "` as c ON (p.ID = c.reference_ID)
	...
	public function stats() {
		global $wpdb;
		return wp_list_pluck($wpdb->get_results("select num, count(*) as ct from (select 0 + if(id = 0, 0, count(ID)) as num from {$wpdb->prefix}hw_yarpp_related_cache group by reference_ID) as t group by num order by num asc", OBJECT_K), 'ct');
	}
	...
	public function graph_data( $threshold = 5 ) {
		...
		$results = $wpdb->get_results("select pair, sum(score) as score from 
			((select concat(reference_ID, '-', ID) as pair, score from {$wpdb->prefix}hw_yarpp_related_cache where reference_ID < ID)
			union
			(select concat(ID, '-', reference_ID) as pair, score from {$wpdb->prefix}hw_yarpp_related_cache where ID < reference_ID)) as t
			group by pair
	...
	public function join_filter($arg) {
		global $wpdb;
		if ($this->yarpp_time)
			$arg .= " join {$wpdb->prefix}" . HW_YARPP_TABLES_RELATED_TABLE . " as yarpp on {$wpdb->posts}.ID = yarpp.ID";
		return $arg;
	}
	...
	public function is_cached($reference_ID) {
		...
		$result = wp_cache_get( 'is_cached_' . $reference_ID, 'hw-yarpp' );
		if ( false !== $result )
			return $result;
		...
		$max_id = $wpdb->get_var("select max(ID) as max_id from {$wpdb->prefix}" . HW_YARPP_TABLES_RELATED_TABLE . " where reference_ID = $reference_ID");

		if ( is_null( $max_id ) )
			return HW_YARPP_NOT_CACHED;
		
		if ( 0 == $max_id )
			$result = HW_YARPP_NO_RELATED;
		else
			$result = HW_YARPP_RELATED;
		
		wp_cache_set( 'is_cached_' . $reference_ID, $result, 'hw-yarpp' );
	...
	public function clear( $reference_IDs ) {
		...
		$wpdb->query("delete from {$wpdb->prefix}" . HW_YARPP_TABLES_RELATED_TABLE . " where reference_ID in (".implode(',',$reference_IDs).")");
		// @since 3.5.2: clear is_cached_* values as well
		foreach ( $reference_IDs as $id )
			wp_cache_delete( 'is_cached_' . $id, 'hw-yarpp' );
	}
	...
	protected function update($reference_ID) {
		...
		$wpdb->query("insert into {$wpdb->prefix}" . HW_YARPP_TABLES_RELATED_TABLE . " (reference_ID,ID,score) " . $this->sql($reference_ID) . " on duplicate key update date = now()");
		...
		if ( $wpdb->rows_affected ) {
			...
			return HW_YARPP_RELATED;
		} else {
			$wpdb->query("insert into {$wpdb->prefix}" . HW_YARPP_TABLES_RELATED_TABLE . " (reference_ID,ID,score) values ($reference_ID,0,0) on duplicate key update date = now()");

			// Clear the caches of those which are no longer related.
			if ( count($original_related) )
				$this->clear($original_related);

			return HW_YARPP_NO_RELATED;
		}
	}

	public function flush() {
		global $wpdb;
		$wpdb->query("truncate table `{$wpdb->prefix}" . HW_YARPP_TABLES_RELATED_TABLE . "`");
	...
	public function related($reference_ID = null, $related_ID = null) {
		...
		if (!is_null($reference_ID) && !is_null($related_ID)) {
			$results = $wpdb->get_col("select ID from {$wpdb->prefix}" . HW_YARPP_TABLES_RELATED_TABLE . " where reference_ID = $reference_ID and ID = $related_ID");
			return count($results) > 0;
		}

		// return a list of ID's of "related" entries
		if ( !is_null($reference_ID) ) {
			return $wpdb->get_col("select distinct ID from {$wpdb->prefix}" . HW_YARPP_TABLES_RELATED_TABLE . " where reference_ID = $reference_ID and ID != 0");
		}

		// return a list of entities which list this post as "related"
		if ( !is_null($related_ID) ) {
			return $wpdb->get_col("select distinct reference_ID from {$wpdb->prefix}" . HW_YARPP_TABLES_RELATED_TABLE . " where ID = $related_ID");
		}
