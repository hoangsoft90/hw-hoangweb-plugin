...
class HW_YARPP_Cache_Postmeta extends HW_YARPP_Cache {
	...
	public function upgrade( $last_version ) {
		if ( $last_version && version_compare('3.4b1', $last_version) > 0 ) {
			...
			delete_post_meta_by_key('_hw_yarpp_title_keywords');
			delete_post_meta_by_key('_hw_yarpp_body_keywords');
		}	
	}
	...
	public function cache_status() {
		...
		return $wpdb->get_var("select (count(p.ID)-sum(m.meta_value IS NULL))/count(p.ID)
			...
			LEFT JOIN `{$wpdb->postmeta}` as m ON (p.ID = m.post_id and m.meta_key = '" . HW_YARPP_POSTMETA_RELATED_KEY . "')
	...
	public function stats() {
		global $wpdb;
		return wp_list_pluck($wpdb->get_results("select num, count(*) as ct from (select 0 + if(meta_value = '" . HW_YARPP_NO_RELATED . "', 0, substring(substring_index(meta_value,':',2),3)) as num from `{$wpdb->postmeta}` where meta_key = '" . HW_YARPP_POSTMETA_RELATED_KEY . "') as t group by num order by num asc", OBJECT_K), 'ct');
	}
	...
	public function begin_yarpp_time($reference_ID) {
		...
		$this->related_postdata = get_post_meta($reference_ID, HW_YARPP_POSTMETA_RELATED_KEY,true);
		if (is_array($this->related_postdata) && count($this->related_postdata))
	...
	// @return HW_YARPP_NO_RELATED | HW_YARPP_RELATED | HW_YARPP_NOT_CACHED
	public function is_cached($reference_ID) {
		$related = get_post_meta($reference_ID, HW_YARPP_POSTMETA_RELATED_KEY, true);
		if ( HW_YARPP_NO_RELATED === $related )
	...
	public function clear( $reference_IDs ) {
		...
		foreach( $reference_IDs as $id ) {
			delete_post_meta( $id, HW_YARPP_POSTMETA_RELATED_KEY );
			delete_post_meta( $id, HW_YARPP_POSTMETA_KEYWORDS_KEY );
		}
	}

	// @return HW_YARPP_NO_RELATED | HW_YARPP_RELATED
	// @used by enforce
	protected function update($reference_ID) {
		...
		if ( count($new_related) ) {
			update_post_meta($reference_ID, HW_YARPP_POSTMETA_RELATED_KEY, $related);
			...
			update_post_meta($reference_ID, HW_YARPP_POSTMETA_RELATED_KEY, HW_YARPP_NO_RELATED);
	...
	public function flush() {
		delete_post_meta_by_key( HW_YARPP_POSTMETA_RELATED_KEY );
		delete_post_meta_by_key( HW_YARPP_POSTMETA_KEYWORDS_KEY );
	}
	...
	public function related($reference_ID = null, $related_ID = null) {
		...
		if (!is_null($reference_ID) && !is_null($related_ID)) {
			$results = get_post_meta($reference_ID, HW_YARPP_POSTMETA_RELATED_KEY,true);
		...
		if (!is_null($reference_ID)) {
			$results = get_post_meta($reference_ID,HW_YARPP_POSTMETA_RELATED_KEY,true);
			if (!$results || $results == HW_YARPP_NO_RELATED)
				return array();
		...
		if (!is_null($related_ID)) {
			return $wpdb->get_col("select post_id from `{$wpdb->postmeta}` where meta_key = '" . HW_YARPP_POSTMETA_RELATED_KEY . "' and meta_value regexp 's:2:\"ID\";s:\d+:\"{$related_ID}\"'");
		}
	...
	private function cache_keywords($ID) {
		...
		update_post_meta($ID, HW_YARPP_POSTMETA_KEYWORDS_KEY, $keywords);
		return $keywords;
	}
	...
	public function get_keywords( $ID, $type = 'all' ) {
		...
		$keywords = get_post_meta($ID, HW_YARPP_POSTMETA_KEYWORDS_KEY, true);
