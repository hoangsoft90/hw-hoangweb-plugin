...
class HW_YARPP_Admin {
	...
	function __construct(&$core) {
		...
		if (isset($_GET['action']) && $_GET['action'] === 'flush' && check_ajax_referer('hw_yarpp_cache_flush', false, false) !== false) {
			$this->core->cache->flush();
			wp_redirect(admin_url('/options-general.php?page=hw_yarpp'));
			die();
		}

		/* If action = copy_templates and the nonce is correct, copy templates */
		if (isset($_GET['action']) && $_GET['action'] === 'copy_templates' && check_ajax_referer('hw_yarpp_copy_templates', false, false) !== false) {
			$this->copy_templates();
			wp_redirect(admin_url('/options-general.php?page=hw_yarpp'));
			die();
		}
	...
	function ajax_register() {
		if (defined('DOING_AJAX') && DOING_AJAX) {
                add_action('wp_ajax_hw_yarpp_display_exclude_terms',   array($this, 'ajax_display_exclude_terms'));
                add_action('wp_ajax_hw_yarpp_display_demo',            array($this, 'ajax_display_demo'));
                add_action('wp_ajax_hw_yarpp_display',                 array($this, 'ajax_display'));
                add_action('wp_ajax_hw_yarpp_optin_data',              array($this, 'ajax_optin_data'));
                add_action('wp_ajax_hw_yarpp_optin_enable',            array($this, 'ajax_optin_enable'));
                add_action('wp_ajax_hw_yarpp_optin_disable',           array($this, 'ajax_optin_disable'));
                add_action('wp_ajax_hw_yarpp_set_display_code',        array($this, 'ajax_set_display_code'));
		}
	}
	...
	function ui_register() {
		...
		if (get_option('hw_yarpp_activated')) {

			delete_option('hw_yarpp_activated');
 			delete_option('hw_yarpp_upgraded');

            /* Optin/Pro message */
            add_action('admin_notices', array($this, 'install_notice'));

		} elseif (get_option('hw_yarpp_upgraded') && current_user_can('manage_options') && $this->core->get_option('optin')) {
			add_action('admin_notices', array($this, 'upgrade_notice'));
		}
		
		if ($this->core->get_option('optin')) delete_option('hw_yarpp_upgraded');
		
		/*
		 * Setup Admin
		 */
        $titleName = 'Bài viết liên quan';
		$this->hook = add_options_page($titleName, $titleName, 'manage_options', 'hw_yarpp', array($this, 'options_page'));
		...
		$metabox_post_types = (array)$this->core->get_option('auto_display_post_types');
        ...
        if(!$this->core->yarppPro['active']){
            foreach ($metabox_post_types as $post_type) {
                if(!$post_type) continue;   //prevent empty
                $optionsUrl    = esc_url(admin_url('options-general.php?page=hw_yarpp'));
                $title  =
                    __('Related Posts' , 'hw-yarpp').
                    '<span class="postbox-title-action">'.
                        '<a href="'.$optionsUrl. '" class="edit-box open-box">'.__('Configure').'</a>'.
                    '</span>';
                add_meta_box('hw_yarpp_relatedposts',$title, array($this, 'metabox'), $post_type, 'normal');
            }
        }
	...
	function settings_screen($current_screen) {
		if ($current_screen->id !== 'settings_page_hw_yarpp') return $current_screen;

		/**
         * @since 3.3: Load options page sections as meta-boxes.
         */
		include_once(HW_YARPP_DIR.'/includes/hw_yarpp_meta_boxes_hooks.php');
		...
		if (is_callable(array($current_screen, 'add_help_tab'))) {
			$current_screen->add_help_tab(array(
				'id'        => 'faq',
				'title'     => __('Frequently Asked Questions', 'hw-yarpp'),
				'callback'  => array(&$this, 'help_faq')
			));

			$current_screen->add_help_tab(array(
				'id'        => 'dev',
				'title'     => __('Developing with YARPP', 'hw-yarpp'),
				'callback'  => array(&$this, 'help_dev')
			));

			$current_screen->add_help_tab(array(
				'id'        => 'optin',
				'title'     => __('Optional Data Collection', 'hw-yarpp'),
	...
	public function help_faq() {
		if (is_null($this->readme)) $this->readme = file_get_contents(HW_YARPP_DIR.'/readme.txt');
		...
		if (preg_match('!== Frequently Asked Questions ==(.*?)^==!sm', $this->readme, $matches)) {
			...
			echo(
                '<a href="https://wordpress.org/extend/plugins/yet-another-related-posts-plugin/faq/">'.
                    __('Frequently Asked Questions', 'hw-yarpp').
                '</a>'
            );
        }
	}
		...
		if (preg_match('!== Developing with YARPP ==(.*?)^==!sm', $this->readme, $matches)) {
			...
			echo(
                '<a href="https://wordpress.org/extend/plugins/yet-another-related-posts-plugin/faq/" target="_blank">'.
                    __('Developing with YARPP', 'hw-yarpp').
                '</a>'
            );
        }
	}
	...
	public function help_optin() {
		echo(
            '<p>'.
                __("With your permission, YARPP will send information about YARPP's settings, usage, and environment
                    back to a central server at ", 'hw-yarpp').'<code>yarpp.org</code>'.'.&nbsp;'.
            '</p>'.
            '<p>'.
		...
		echo(
            '<p>'.
                __("The following information is sent back to YARPP:", 'hw-yarpp').
            '</p>'.
		    '<div id="optin_data_frame"></div>'.
            '<p>'.
                __("In addition, YARPP also loads an invisible pixel image with your YARPP results to know how often YARPP is being used.", 'hw-yarpp').
            '</p>'
        );
	}
	...
	function the_optin_button($action, $echo = false) {
        ...
        if ($action === 'disable'){
            $out =
            '<a id="hw-yarpp-optin-button'.$status.'" class="button" '.$status.'>'.
	...
	function the_donothing_button($msg, $echo = false) {
        ...
        $out ='<a href="options-general.php?page=hw-yarpp" class="button">'.$msg.'</a>';
	...
	function optin_button_script($optinAction, $echo=false) {
		wp_nonce_field('hw_yarpp_optin_'.$optinAction, 'hw_yarpp_optin-nonce', false);

        ob_start();
           include(HW_YARPP_DIR.'/includes/optin_notice.js.php');
           $out = ob_get_contents();
        ob_end_clean();
	...
	public function optin_notice($type=false, $optinAction) {
		...
		if(is_null($screen) || $screen->id == 'settings_page_hw_yarpp') return;
		...
		switch($type) {
			...
			default:
				...
				update_user_option($user, 'hw_yarpp_saw_optin', true);
		}
		...
		if ($type === 'install'){
            $tmp  = __('Thank you for installing <span>Yet Another Related Posts Plugin</span>!', 'hw-yarpp');
        ...
        if($this->core->yarppPro['active']){
            ...
            /*$out .=
                '<p>'.
                    'You currently have <strong>HW YARPP Basic</strong> and <strong>YARPP Pro</strong> enabled.<br/><br/>'.
                    '<a href="options-general.php?page=hw_yarpp" class="button">Take me to the settings page</a>'.
                '</p>';
            */
	...
	public function render_screen_settings ($output, $current_screen) {
		if ( $current_screen->id != 'settings_page_hw_yarpp' )
			return $output;

		$output .= "<div id='hw_yarpp_extra_screen_settings'><label for='hw_yarpp_display_code'><input type='checkbox' name='hw_yarpp_display_code' id='hw_yarpp_display_code'";
		$output .= checked($this->core->get_option('display_code'), true, false);
		$output .= " />";
		$output .= __('Show example code output', 'hw-yarpp');
		$output .= '</label></div>';
	...
	public function enqueue() {
		$version = defined('WP_DEBUG') && WP_DEBUG ? time() : HW_YARPP_VERSION;
		$screen = get_current_screen();
		if (!is_null($screen) && $screen->id === 'settings_page_hw_yarpp') {

            wp_enqueue_style('hw_yarpp_switch_options',  plugins_url('style/options_switch.css', dirname(__FILE__)), array(), $version );
            wp_enqueue_script('hw_yarpp_switch_options', plugins_url('js/options_switch.js', dirname(__FILE__)), array('jquery'), $version );

            wp_enqueue_style('wp-pointer');
            wp_enqueue_style('hw_yarpp_options', plugins_url('style/options_basic.css', dirname(__FILE__)), array(), $version );

            wp_enqueue_script('postbox');
            wp_enqueue_script('wp-pointer');
            wp_enqueue_script('hw_yarpp_options', plugins_url('js/options_basic.js', dirname(__FILE__)), array('jquery'), $version );

		}

		$metabox_post_types = (array)$this->core->get_option('auto_display_post_types');
		if (!is_null($screen) && ($screen->id == 'post' || in_array( $screen->id, $metabox_post_types))) {
			wp_enqueue_script('hw_yarpp_metabox', plugins_url('js/metabox.js', dirname(__FILE__)), array('jquery'), $version );
		}
	}
	
	public function settings_link($links, $file) {
		$this_plugin = dirname(plugin_basename(dirname(__FILE__))).'/hw-yarpp.php'; //since hw moved this plugin into hw module
		if($file == $this_plugin) {
			$links[] = '<a href="options-general.php?page=hw_yarpp">'.__('Settings').'</a>';
		}
		return $links;
	}
	...
	public function options_page() {
        ...
        if ($mode !== 'basic' && ($mode === 'pro' || $this->core->yarppPro['active'])){
            include_once(HW_YARPP_DIR.'/includes/hw_yarpp_pro_options.php');
        } else {
            include_once(HW_YARPP_DIR . '/includes/hw_yarpp_options.php');
        }
	}
	...
	public function metabox() {
		?>
		<style>
		#hw_yarpp_relatedposts h3 .postbox-title-action {
			right: 30px;
			top: 5px;
			position: absolute;
			padding: 0;
		}
		#hw_yarpp_relatedposts:hover .edit-box {
			display: inline;
		}
		</style>
		<?php
		if ( !get_the_ID() ) {
			echo "<div><p>".__("Related entries may be displayed once you save your entry",'hw-yarpp').".</p></div>";
		} else {
			wp_nonce_field( 'hw_yarpp_display', 'hw_yarpp_display-nonce', false );
			echo '<div id="hw-yarpp-related-posts"><img src="' . esc_url( admin_url( 'images/wpspin_light.gif' ) ) . '" alt="" /></div>';
		}
	}
	
	// @since 3.3: default metaboxes to show:
	public function default_hidden_meta_boxes($hidden, $screen) {
		if ($screen->id === 'settings_page_hw_yarpp') {
	...
	public function ajax_display_exclude_terms() {
		check_ajax_referer('hw_yarpp_display_exclude_terms');
...
//		if ('category' === $taxonomy) $exclude .= ','.get_option('default_category');
	...
	public function ajax_display() {
		check_ajax_referer('hw_yarpp_display');
	...
	public function ajax_display_demo() {
		check_ajax_referer('hw_yarpp_display_demo');
	...
	public function ajax_optin_data() {
		check_ajax_referer('hw_yarpp_optin_data');
    ...
    public function ajax_optin_disable() {
		check_ajax_referer('hw_yarpp_optin_disable');
	...
	public function ajax_optin_enable() {
		check_ajax_referer('hw_yarpp_optin_enable');
	...
	public function ajax_set_display_code() {
		check_ajax_referer( 'hw_yarpp_set_display_code' );
