...
abstract class HW_YARPP_Cache {
	...
	function __construct( &$core ) {
		$this->core = &$core;
		$this->name = __($this->name, 'hw-yarpp');
	}
	 ...
	 * return HW_YARPP_NO_RELATED | HW_YARPP_RELATED | HW_YARPP_DONT_RUN | false if no good input
     */
	function enforce($reference_ID, $force = false) {
		...
		$status = apply_filters('hw_yarpp_cache_enforce_status', $status, $reference_ID);
	
		// There's a stop signal:
		if ($status === HW_YARPP_DONT_RUN) return HW_YARPP_DONT_RUN;
	
		// If not cached, process now:
		if ($status === HW_YARPP_NOT_CACHED || $force) $status = $this->update((int) $reference_ID); // status now will be HW_YARPP_NO_RELATED | HW_YARPP_RELATED

		// There are no related posts
		if ($status === HW_YARPP_NO_RELATED) return HW_YARPP_NO_RELATED;
     ...
     * @return string HW_YARPP_NO_RELATED | HW_YARPP_RELATED | HW_YARPP_NOT_CACHED
     */
	...
	protected function title_keywords($ID, $max = 20) {
		return apply_filters('hw_yarpp_title_keywords', $this->extract_keywords(get_the_title($ID), $max, $ID), $max, $ID);
	}
	...
	protected function body_keywords($ID, $max = 20) {
		...
		return apply_filters('hw_yarpp_body_keywords', $this->extract_keywords($content, $max, $ID), $max, $ID);
	}
	
	private function extract_keywords( $html, $max = 20, $ID = 0 ) {
	
		/**
		 * @filter hw_yarpp_extract_keywords
		 *
		 ...
		 * @param (int) $ID
		 */
		if ($keywords = apply_filters('hw_yarpp_extract_keywords', false, $html, $max, $ID)) return $keywords;
		...
		$words_file = HW_YARPP_DIR.'/lang/words-'.$lang.'.php';
		...
		$overusedwords = apply_filters( 'hw_yarpp_keywords_overused_words', $overusedwords );
	 ...
	 * It can be modified via the hw_yarpp_blacklist and hw_yarpp_blackmethods filters.
	 */
	/* blacklisted so far:
		- diggZ-Et
		- reddZ-Et
	...
	*/
	function white( $filter ) {
		...
		if ( is_null($blacklist) || is_null($blackmethods) ) {
			...
			$blacklist = (array) apply_filters( 'hw_yarpp_blacklist', $yarpp_blacklist );
			$blackmethods = (array) apply_filters( 'hw_yarpp_blackmethods', $yarpp_blackmethods );
		}
		
		if ( is_array($filter) && is_a( $filter[0], 'HW_YARPP' ) )
			return false;
	...
	/* FYI, apply_filters_if_white was used here to avoid a loop in apply_filters('the_content') > HW_YARPP::the_content() > HW_YARPP::related() > HW_YARPP_Cache::body_keywords() > apply_filters('the_content').*/
