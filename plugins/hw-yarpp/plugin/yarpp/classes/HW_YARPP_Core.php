...
class HW_YARPP {
	...
	public function __construct() {
		...
		load_plugin_textdomain('hw-yarpp', false, plugin_basename(HW_YARPP_DIR).'/lang');

		/* Load cache object. */
		$this->storage_class    = 'HW_YARPP_Cache_'.ucfirst(HW_YARPP_CACHE_TYPE);
		$this->cache            = new $this->storage_class($this);
		$this->cache_bypass     = new HW_YARPP_Cache_Bypass($this);
		...
		if (!get_option('hw_yarpp_version')) update_option('hw_yarpp_activated', true);

        /**
		 * @since 3.4 Only load UI if we're in the admin.
         */
		if (is_admin()) {
			require_once(HW_YARPP_DIR.'/classes/HW_YARPP_Admin.php');
			$this->admin = new HW_YARPP_Admin($this);
			$this->enforce();
		}
	}
	...
	private function load_default_options() {
		$this->default_options = array(
			...
			'before_post' => ' <small>',
			'after_post' => '</small>',
			'before_related' => '<h3>'.__('Related posts:','hw-yarpp').'</h3><ol>',
			'after_related' => '</ol>',
			'no_results' => '<p>'.__('No related posts.','hw-yarpp').'</p>',
			'order' => 'score DESC',
			'rss_limit' => 3,
			...
			'rss_before_related' => '<h3>'.__('Related posts:','hw-yarpp').'</h3><ol>',
			'rss_after_related' => '</ol>',
			'rss_no_results' => '<p>'.__('No related posts.','hw-yarpp').'</p>',
			...
			'require_tax' => array(),
			'optin' => false,
			'thumbnails_heading' => __('Related posts:','hw-yarpp'),
			'thumbnails_default' => plugins_url('images/default.png', dirname(__FILE__)),
			'rss_thumbnails_heading' => __('Related posts:','hw-yarpp'),
	...
	public function set_option($options, $value = null) {
		...
		update_option('hw-yarpp', $new_options);
	...
	public function get_option($option = null) {
		$options = (array) get_option('hw-yarpp', array());
    ...
    private function get_pro_options(){
        $current  = get_option('hw_yarpp_pro');
        $defaults = $this->load_pro_default_options();

        if ($current) {
            $out = array_merge($defaults,$current);
            update_option('hw_yarpp_pro', $out);
        } else {
            $out = $defaults;
            add_option('hw_yarpp_pro',$out);
        }
	...
	public function activate() {
		...
		if (!get_option('hw_yarpp_version')) {
			add_option('hw_yarpp_version', HW_YARPP_VERSION);
	...
	function diagnostic_fulltext_disabled() {
		return get_option('hw_yarpp_fulltext_disabled', false);
	}
	...
	public function enable_fulltext() {
		...
		$wpdb->query("ALTER TABLE $wpdb->posts ADD FULLTEXT `hw_yarpp_title` (`post_title`)");
		if (!empty($wpdb->last_error)){
            $this->disable_fulltext();
            return false;
        }

		$wpdb->query("ALTER TABLE $wpdb->posts ADD FULLTEXT `hw_yarpp_content` (`post_content`)");
		if (!empty($wpdb->last_error)){
            $this->disable_fulltext();
            return false;
        }
	...
	public function disable_fulltext() {
		if ((bool) get_option('hw_yarpp_fulltext_disabled', false) === true) return;
		...
		update_option('hw_yarpp_fulltext_disabled', true);
	}
	...
	public function diagnostic_fulltext_indices() {
		global $wpdb;
		$wpdb->get_results("SHOW INDEX FROM {$wpdb->posts} WHERE Key_name = 'hw_yarpp_title' OR Key_name = 'hw_yarpp_content'");
	...
	public function diagnostic_hidden_metaboxes() {
		...
		$raw = $wpdb->get_var(
            ...
            "WHERE meta_key = 'metaboxhidden_settings_page_hw_yarpp' ".
	...
	public function diagnostic_generate_thumbnails() {
		return (defined('HW_YARPP_GENERATE_THUMBNAILS') && HW_YARPP_GENERATE_THUMBNAILS);
	}
	...
	public function thumbnail_dimensions() {
		...
		if (!isset($_wp_additional_image_sizes['hw-yarpp-thumbnail'])) return $this->default_dimensions;

		$dimensions = $_wp_additional_image_sizes['hw-yarpp-thumbnail'];
		$dimensions['size'] = 'hw-yarpp-thumbnail';
	...
	public function maybe_enqueue_thumbnails() {
		...
		if (!in_array(get_post_type(), (array)$auto_display_post_types)) return;
	...
	public function enqueue_thumbnails($dimensions) {
		...
		wp_enqueue_style("hw-yarpp-thumbnails-".$dimensions['size'], $url, array(), HW_YARPP_VERSION, 'all');
	}
	...
	public function get_templates() {
		if (is_null($this->templates)) {
			$this->templates = glob(STYLESHEETPATH.'/hw-yarpp-template-*.php');
	...
	public function upgrade() {
		$last_version = get_option('hw_yarpp_version');

		if (version_compare(HW_YARPP_VERSION, $last_version) === 0) return;
		...
		update_option('hw_yarpp_version', HW_YARPP_VERSION);
		update_option('hw_yarpp_upgraded', true);
		$this->delete_transient('hw_yarpp_optin');
	}
	...
	public function upgrade_3_4b2() {
		...
		foreach ($yarpp_3_3_options as $key => $default) {
			$value = get_option("hw_yarpp_$key", null);
		...
		update_option('hw-yarpp', $yarpp_options);
		...
		if (count($option_keys)) {
			$in = "('hw_yarpp_".join("', 'hw_yarpp_", $option_keys)."')";
	...
	public function upgrade_3_4b5() {
		...
		update_option('hw-yarpp', $options);
	}
	...
	public function upgrade_3_4_4b2() {
		...
		$weight_map = array(2 => 1, 3 => HW_YARPP_EXTRA_WEIGHT);
    ...
    public function optin_data() {
		...
		$data = array(
			'versions' => array(
				'hw_yarpp' => HW_YARPP_VERSION,
				'wp'    => get_bloginfo('version'),
				'php'   => phpversion()
			),
			'hw_yarpp' => array(
				'settings'      => array_intersect_key($settings, $collect),
				'cache_engine'  => HW_YARPP_CACHE_TYPE
			),
			'diagnostics' => array(
		...
		$data['hw_yarpp']['settings']['auto_display_post_types'] = implode('|',$settings['auto_display_post_types']);
		...
		foreach (array('before_related','rss_before_related') as $key) {
			if ($settings[$key] !== '<p>'.__('Related posts:','hw-yarpp').'</p><ol>'
		...
		$data['hw_yarpp']['changed_settings'] = implode('|', $changed);
		
		if (method_exists($this->cache, 'cache_status')) $data['hw_yarpp']['cache_status'] = $this->cache->cache_status();
        ...
        if (method_exists($this->cache, 'stats')) {
			...
			$data['hw_yarpp']['stats'] = implode('|', $flattened);
		}
    ...
    protected function display_basic(){
        /* if it's not an auto-display post type, return */
        if (!in_array(get_post_type(), (array)$this->get_option('auto_display_post_types'))) return null;
        ...
        $post_types = apply_filters('hw_yarpp_map_post_types', $post_types, 'website');
    ...
    public function display_pro($domain) {
        ...
        ob_start();
        include(HW_YARPP_DIR.'/includes/phtmls/hw_yarpp_pro_tag'.$ssp.'.phtml');
	...
	public function display_related($reference_ID = null, $args = array(), $echo = true) {
        ...
        $this->enforce();
        wp_enqueue_style('hw-yarppRelatedCss', HW_YARPP_URL.'/style/related.css');
        $output = null;
        ...
        if ($cache_status === HW_YARPP_DONT_RUN) return;
        if ($cache_status !== HW_YARPP_NO_RELATED) $this->active_cache->begin_yarpp_time($reference_ID, $args);

        $this->save_post_context();

        global $wp_query; $wp_query = new WP_Query();

        if ($cache_status !== HW_YARPP_NO_RELATED) {
        ...
        $wp_query->posts = apply_filters('hw_yarpp_results', $wp_query->posts, array(
        ...
        $output .= "<div class='";
        if ($domain === 'website') {
            $output .= "hw-yarpp-related";
        } else {
            $output .= "hw-yarpp-related-{$domain}";
        }

        if ($related_count < 1) {
            $output .= " hw-yarpp-related-none";
        }

        $output .= "'>\n";

        if ($domain === 'metabox') {
            include(HW_YARPP_DIR.'/includes/template_metabox.php');
        } else if ((bool) $template && $template === 'thumbnails') {
            include(HW_YARPP_DIR.'/includes/template_thumbnails.php');
            ...
            include(HW_YARPP_DIR.'/includes/template_widget.php');
        } else {
            include(HW_YARPP_DIR.'/includes/template_builtin.php');
        }
	...
	public function get_related($reference_ID = null, $args = array()) {
		...
		$cache_status = $this->active_cache->enforce($reference_ID);
		if ($cache_status === HW_YARPP_DONT_RUN || $cache_status === HW_YARPP_NO_RELATED) return array();
	...
	public function related_exist($reference_ID = null, $args = array()) {
		...
		if ($cache_status === HW_YARPP_NO_RELATED) return false;
	...
	public function the_content_feed($content) {
		...
		$post_types = apply_filters('hw_yarpp_map_post_types', $post_types, 'rss');
	...
	public function version_info($enforce_cache = false) {
		if (!$enforce_cache && false !== ($result = $this->get_transient('hw_yarpp_version_info'))) return $result;

		$version = HW_YARPP_VERSION;
		...
		if (is_wp_error($remote) || wp_remote_retrieve_response_code($remote) != 200 || !isset($remote['body'])){
			$this->set_transient('hw_yarpp_version_info', null, 60*60);
			return false;
        }
		
		if ($result = @unserialize($remote['body'])) $this->set_transient('hw_yarpp_version_info', $result, 60*60*24);
     ...
     */
	public function __unused_optin_ping() {
		if ($this->get_transient('hw_yarpp_optin')) return true;
		...
		if (is_wp_error($remote)
			...
			/* try again later */
			$this->set_transient('hw_yarpp_optin', null, 60*60);
			return false;
		}

		$this->set_transient('hw_yarpp_optin', null, 60*60*24*7);
