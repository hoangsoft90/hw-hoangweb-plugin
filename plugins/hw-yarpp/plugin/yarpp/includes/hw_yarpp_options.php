...
if (!$hw_yarpp->diagnostic_custom_templates()) {
    ...
    if ($template_option !== false &&  $template_option !== 'thumbnails') hw_yarpp_set_option('template', false);

    $template_option = hw_yarpp_get_option('rss_template');
    if ($template_option !== false && $template_option !== 'thumbnails') hw_yarpp_set_option('rss_template', false);
}
    ...
    if ($yarpp_version_info['result'] === 'new') {
        ...
        printf(
            __(
               ...
               'or <a href="%5$s">update automatically</a>.', 'hw-yarpp'),
            $plugin_name,
            esc_url($details_url),
        ...
        printf(
            __(
                ...
                "You can <a href=\"%s\">download it here</a> at your own risk.", "hw-yarpp"),
...
/* MyISAM Check */
include 'hw_yarpp_myisam_notice.php';

/* This is not a yarpp pluging update, it is an yarpp option update */
if (isset($_POST['update_hw_yarpp']) && check_admin_referer('update_hw_yarpp', 'update_hw_yarpp-nonce')) {
    $new_options = array();
    foreach ($hw_yarpp->default_options as $option => $default) {
        if ( is_bool($default) )
    ...
    if ( isset($_POST['weight']) ) {
        ...
        foreach ( (array) $_POST['weight'] as $key => $value) {
            ...
            if ( $value == 'consider_extra' )
                $new_options['weight'][$key] = HW_YARPP_EXTRA_WEIGHT;
        }
        foreach ( (array) $_POST['weight']['tax'] as $tax => $value) {
            if ( $value == 'consider' )
                $new_options['weight']['tax'][$tax] = 1;
            if ( $value == 'consider_extra' )
                $new_options['weight']['tax'][$tax] = HW_YARPP_EXTRA_WEIGHT;
