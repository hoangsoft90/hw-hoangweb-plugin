 ...
 * If you want to create a new template, look at yarpp-templates/hw-yarpp-template-example.php as an example.
...
if (have_posts()) {
	$output .= '<div class="hw-yarpp-thumbnails-horizontal">' . "\n";
	while (have_posts()) {
		the_post();

		$output .= "<a class='hw-yarpp-thumbnail' href='" . get_permalink() . "' title='" . the_title_attribute('echo=0') . "'>" . "\n";
		...
		if ( trim($post_thumbnail_html) != '' )
			...
			$output .= '<span class="hw-yarpp-thumbnail-default"><img src="' . esc_url($thumbnails_default) . '"/></span>';

		$output .= '<span class="hw-yarpp-thumbnail-title">' . get_the_title() . '</span>';
		$output .= '</a>' . "\n";
