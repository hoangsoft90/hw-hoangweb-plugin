<?php
//register metabox for hw_pt_condition post type edit page
include_once('dynamic-conditions-metabox.php');

if(class_exists('AdminPageFramework_PostType')):
/**
 * Class HW_Conditions_Manager
 */
class HW_Conditions_Manager extends AdminPageFramework_PostType
{
    const post_type = 'hw_pt_condition';
     ...
     * @return array
     */
    public function columns_hw_pt_condition( $aHeaderColumns ) {//columns_{post type slug}      #hwml_shortcodes_list
     ...
     * @return string
     */
    public function cell_hw_pt_condition_desc( $sCell, $iPostID ) { // cell_{post type}_{column key}
     ...
     * @param $iPostID
     */
    public function cell_hw_pt_condition_enable( $sCell, $iPostID ) {
     ...
     * @param $iPostID
     */
    public function cell_hw_pt_condition_template( $sCell, $iPostID ) {
        $out = '<ul>';
     ...
     * @return string
     */
    public function style_HW_Conditions_Manager( $sStyle ) { #style_{instantiated class name}()
    ...
    public static function init_metabox() {
        if(is_admin() ){
            global $current_user;
            if(class_exists('HW_Conditions_Manager') && class_exists('HW_Conditions_Metabox')
                && ( current_user_can('administrator') )    //only for administrator
            ){
                new HW_Conditions_Manager( HW_Conditions_Manager::post_type );  // the post type slug

                HW_Conditions_Metabox::setInstance( new HW_Conditions_Metabox(
                    null,   // meta box ID - can be null.
                    __( 'Thêm điều kiện' ), // title
                    array( HW_Conditions_Manager::post_type ),  // post type slugs: post, page, etc.
                    'normal',   // context
                    'low'   // priority
                ));
                //add new metabox
                new HW_Conditions_Metabox(
                    null,   // meta box ID - can be null.
                    __( 'Hướng dẫn' ), // title
                    array( HW_Conditions_Manager::post_type ),  // post type slugs: post, page, etc.
                    'normal',   // context
    ...
    /**
     * return admin page for post type self::post_type
     * @return string|void
     */
    public static function admin_condition_mananger_link() {
        return admin_url('edit.php?post_type='. self::post_type);
    }
}
    #add_action('plugins_loaded', 'HW_Conditions_Manager::init_metabox');
endif;
