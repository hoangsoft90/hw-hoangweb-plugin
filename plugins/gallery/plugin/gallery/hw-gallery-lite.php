...
class HW_Gallery_Lite {
     ...
     * @var string
     */
    public $plugin_name = 'Gallery Lite';
     ...
     * @var string
     */
    public $plugin_slug = 'hw-gallery-lite';
...
//register_activation_hook( __FILE__, 'hw_gallery_lite_activation_hook' );
/**
 ...
 */
function hw_gallery_lite_activation_hook( $network_wide ) {

    global $wp_version;
    if ( version_compare( $wp_version, '3.8', '<' ) && ! defined( 'ENVIRA_FORCE_ACTIVATION' ) ) {
        deactivate_plugins( plugin_basename( __FILE__ ) );
        wp_die( sprintf( __( 'Sorry, but your version of WordPress does not meet Envira Gallery\'s required version of <strong>3.8</strong> to run properly. The plugin has been deactivated. <a href="%s">Click here to return to the Dashboard</a>.', 'hw-gallery' ), get_admin_url() ) );
    }

}

// Load the main plugin class.
$hw_gallery_lite = HW_Gallery_Lite::get_instance();

// Conditionally load the template tag.
if ( ! function_exists( 'hw_gallery' ) ) {
    /**
     ...
     */
    function hw_gallery( $id, $type = 'id', $args = array(), $return = false ) {
        ...
        // Build the shortcode.
        $shortcode = ! empty( $args_string ) ? '[hw-gallery ' . $type . '="' . $id . '"' . $args_string . ']' : '[hw-gallery ' . $type . '="' . $id . '"]';
