...
add_action( 'wp_ajax_hw_gallery_load_image', 'hw_gallery_lite_ajax_load_image' );
/**
 * Loads an image into a gallery.
 *
 * @since 1.0.0
 */
function hw_gallery_lite_ajax_load_image() {

    // Run a security check first.
    check_ajax_referer( 'hw-gallery-load-image', 'nonce' );
    ...
    $gallery_data = hw_gallery_lite_ajax_prepare_gallery_data( $gallery_data, $id );
    update_post_meta( $post_id, '_eg_gallery_data', $gallery_data );

    // Run hook before building out the item.
    do_action( 'hw_gallery_ajax_load_image', $id, $post_id );

    // Build out the individual HTML output for the gallery image that has just been uploaded.
    $html = HW_Gallery_Metaboxes_Lite::get_instance()->get_gallery_item( $id, $gallery_data['gallery'][$id], $post_id );
...
add_action( 'wp_ajax_hw_gallery_load_library', 'hw_gallery_lite_ajax_load_library' );
/**
 * Loads the Media Library images into the media modal window for selection.
 *
 * @since 1.0.0
 */
function hw_gallery_lite_ajax_load_library() {

    // Run a security check first.
    check_ajax_referer( 'hw-gallery-load-gallery', 'nonce' );
    ...
    if ( $library ) {
        foreach ( (array) $library as $image ) {
            ...
            $class       = $has_gallery && in_array( $post_id, (array) $has_gallery ) ? ' selected hw-gallery-in-gallery' : '';
...
add_action( 'wp_ajax_hw_gallery_library_search', 'hw_gallery_lite_ajax_library_search' );
/**
 ...
 * @deprecated 1.2.6
 */
function hw_gallery_lite_ajax_library_search() {

    // Search is now performed by hw_gallery_lite_ajax_load_library()
    // Return the result of that function, so addons and custom code relying on this action
    // doesn't break
    hw_gallery_lite_ajax_load_library();
    die;

}

add_action( 'wp_ajax_hw_gallery_insert_images', 'hw_gallery_lite_ajax_insert_images' );
/**
 * Inserts one or more images from the Media Library into a gallery.
 *
 * @since 1.0.0
 */
function hw_gallery_lite_ajax_insert_images() {

    // Run a security check first.
    check_ajax_referer( 'hw-gallery-insert-images', 'nonce' );
    ...
    do_action( 'hw_gallery_ajax_insert_images', $images, $post_id );
...
add_action( 'wp_ajax_hw_gallery_sort_images', 'hw_gallery_lite_ajax_sort_images' );
/**
 * Sorts images based on user-dragged position in the gallery.
 *
 * @since 1.0.0
 */
function hw_gallery_lite_ajax_sort_images() {

    // Run a security check first.
    check_ajax_referer( 'hw-gallery-sort', 'nonce' );
...
add_action( 'wp_ajax_hw_gallery_remove_image', 'hw_gallery_lite_ajax_remove_image' );
/**
 * Removes an image from a gallery.
 *
 * @since 1.0.0
 */
function hw_gallery_lite_ajax_remove_image() {

    // Run a security check first.
    check_ajax_referer( 'hw-gallery-remove-image', 'nonce' );
    ...
    do_action( 'hw_gallery_ajax_remove_images', $attach_id, $post_id );
...
add_action( 'wp_ajax_hw_gallery_save_meta', 'hw_gallery_lite_ajax_save_meta' );
/**
...
add_action( 'wp_ajax_hw_gallery_refresh', 'hw_gallery_lite_ajax_refresh' );
/**
...
add_action( 'wp_ajax_hw_gallery_load_gallery_data', 'hw_gallery_lite_ajax_load_gallery_data' );
/**
 * Retrieves and return gallery data for the specified ID.
 *
 * @since 1.0.0
 */
function hw_gallery_lite_ajax_load_gallery_data() {
 ...
 */
function hw_gallery_lite_ajax_prepare_gallery_data( $gallery_data, $id ) {
    ...
    $gallery_data = apply_filters( 'hw_gallery_ajax_item_data', $gallery_data, $attachment, $id );
