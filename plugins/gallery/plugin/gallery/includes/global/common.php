...
class HW_Gallery_Common_Lite {
    ...
    public function get_columns() {
        ...
        $columns = array(
            array(
                'value' => '1',
                'name'  => __( 'One Column (1)', 'hw-gallery' )
            ),
            array(
                'value' => '2',
                'name'  => __( 'Two Columns (2)', 'hw-gallery' )
            ),
            array(
                'value' => '3',
                'name'  => __( 'Three Columns (3)', 'hw-gallery' )
            ),
            array(
                'value' => '4',
                'name'  => __( 'Four Columns (4)', 'hw-gallery' )
            ),
            array(
                'value' => '5',
                'name'  => __( 'Five Columns (5)', 'hw-gallery' )
            ),
            array(
                'value' => '6',
                'name'  => __( 'Six Columns (6)', 'hw-gallery' )
            )
        );

        return apply_filters( 'hw_gallery_columns', $columns );
    ...
    public function get_gallery_themes() {
        ...
        $themes = array(
            array(
                'value' => 'base',
                'name'  => __( 'Base', 'hw-gallery' ),
                'file'  => $this->base->file
            )
        );

        return apply_filters( 'hw_gallery_gallery_themes', $themes );
    ...
    public function get_lightbox_themes() {
        ...
        return apply_filters( 'hw_gallery_lightbox_themes', $themes );
    ...
    public function get_title_displays() {
        ...
        $displays = array(
            array(
                'value' => 'float',
                'name'  => __( 'Float', 'hw-gallery' )
            ),
            array(
                'value' => 'float_wrap',
                'name'  => __( 'Float (Wrapped)', 'hw-gallery' )
            ),
        );

        return apply_filters( 'hw_gallery_title_displays', $displays );
    ...
    public function get_config_default( $key ) {
        ...
        $defaults = apply_filters( 'hw_gallery_defaults', $defaults, $post_id );
    ...
    public function resize_image( $url, $width = null, $height = null, $crop = true, $align = 'c', $quality = 100, $retina = false ) {
        ...
        if ( ! file_exists( $dest_file_name ) ) {
            ...
            if ( is_wp_error( $editor ) ) {
                return new WP_Error( 'hw-gallery-error-no-editor', __( 'No image editor could be selected. Please verify with your webhost that you have either the GD or Imagick image library compiled with your PHP install on your server.', 'hw-gallery' ) );
            }
    ...
    public function get_image_info( $args ) {
        ...
        if ( empty( $url ) ) {
            return new WP_Error( 'hw-gallery-error-no-url', __( 'No image URL specified for cropping.', 'hw-gallery' ) );
        }
        ...
        if ( ! file_exists( $file_path ) ) {
            return new WP_Error( 'hw-gallery-error-no-file', __( 'No file could be found for the image URL specified.', 'hw-gallery' ) );
        }
        ...
        if ( ! $size ) {
            return new WP_Error( 'hw-gallery-error-no-size', __( 'The dimensions of the original image could not be retrieved for cropping.', 'hw-gallery' ) );
        }
    ...
    public function flush_gallery_caches( $post_id, $slug = '' ) {
        ...
        do_action( 'hw_gallery_flush_caches', $post_id, $slug );
...
$hw_gallery_common_lite = HW_Gallery_Common_Lite::get_instance();