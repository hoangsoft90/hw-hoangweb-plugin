 ...
 */
class HW_Gallery_Shortcode_Lite {
    ...
    public function shortcode( $atts ) {
        ...
        if ( empty( $atts ) ) {
            ...
            $data = apply_filters( 'hw_gallery_custom_gallery_data', false, $atts, $post );
        }
        ...
        $data = apply_filters( 'hw_gallery_pre_data', $data, $gallery_id );
        ...
        do_action( 'hw_gallery_before_output', $data );

        // Apply a filter before starting the gallery HTML.
        $gallery = apply_filters( 'hw_gallery_output_start', $gallery, $data );

        // Build out the gallery HTML.
        $gallery .= '<div id="hw-gallery-wrap-' . sanitize_html_class( $data['id'] ) . '" class="' . $this->get_gallery_classes( $data ) . '">';
            $gallery  = apply_filters( 'hw_gallery_output_before_container', $gallery, $data );
            $gallery .= '<div id="hw-gallery-' . sanitize_html_class( $data['id'] ) . '" class="hw-gallery-public hw-gallery-' . sanitize_html_class( $this->get_config( 'columns', $data ) ) . '-columns hw-gallery-clear isotope" data-hw-gallery-columns="' . $this->get_config( 'columns', $data ) . '">';
                foreach ( $data['gallery'] as $id => $item ) {
                    ...
                    $gallery  = apply_filters( 'hw_gallery_output_before_item', $gallery, $id, $item, $data, $i );
                    $output   = '<div id="hw-gallery-item-' . sanitize_html_class( $id ) . '" class="' . $this->get_gallery_item_classes( $item, $i, $data ) . '" style="margin-bottom: ' . $this->get_config( 'margin', $data ) . 'px;" ' . apply_filters( 'hw_gallery_output_item_attr', '', $id, $item, $data, $i ) . '>';

                    if ( ! empty( $item['link'] ) ) {
                        $output  = apply_filters( 'hw_gallery_output_before_link', $output, $id, $item, $data, $i );
                        $output .= '<a href="' . esc_url( $item['link'] ) . '" class="hw-gallery-' . sanitize_html_class( $data['id'] ) . ' hw-gallery-link" rel="hwgallery' . sanitize_html_class( $data['id'] ) . '" title="' . esc_attr( $item['title'] ) . '" data-thumbnail="' . esc_url( $item['thumb'] ) . '">';
                    }

                            $output  = apply_filters( 'hw_gallery_output_before_image', $output, $id, $item, $data, $i );
                            $output .= '<img id="hw-gallery-image-' . sanitize_html_class( $id ) . '" class="hw-gallery-image hw-gallery-preload hw-gallery-image-' . $i . '" src="' . esc_url( plugins_url( 'assets/css/images/holder.gif', dirname( dirname( __FILE__ ) ) ) ) . '" data-hw-gallery-src="' . esc_url( $imagesrc ) . '" alt="' . esc_attr( $item['alt'] ) . '" title="' . esc_attr( $item['title'] ) . '" ' . apply_filters( 'hw_gallery_output_image_attr', '', $id, $item, $data, $i ) . ' />';
                            $output  = apply_filters( 'hw_gallery_output_after_image', $output, $id, $item, $data, $i );

                        if ( ! empty( $item['link'] ) ) {
                            $output .= '</a>';
                            $output  = apply_filters( 'hw_gallery_output_after_link', $output, $id, $item, $data, $i );
                        }

                    $output .= '</div>';
                    $output  = apply_filters( 'hw_gallery_output_single_item', $output, $id, $item, $data, $i );
                    $gallery .= $output;
                    $gallery  = apply_filters( 'hw_gallery_output_after_item', $gallery, $id, $item, $data, $i );

                    // Increment the iterator.
                    $i++;
                }
            $gallery .= '</div>';
            $gallery  = apply_filters( 'hw_gallery_output_after_container', $gallery, $data );
        $gallery .= '</div>';
        $gallery  = apply_filters( 'hw_gallery_output_end', $gallery, $data );

        // Increment the counter.
        $this->counter++;

        // Return the gallery HTML.
        return apply_filters( 'hw_gallery_output', $gallery, $data );
    ...
    public function gallery_init() {
        ...
        <script type="text/javascript">jQuery(document).ready(function($){<?php ob_start();
            do_action( 'hw_gallery_api_start_global' );
            foreach ( $this->data as $data ) :
                ...
                do_action( 'hw_gallery_api_start', $data ); ?>

                var hw_gallery_container_<?php echo $data['id']; ?> = $('#hw-gallery-<?php echo $data['id']; ?>'),
                    hw_gallery_on_show_<?php echo $data['id']; ?>,
                    hw_gallery_on_render_<?php echo $data['id']; ?>,
                    hw_gallery_holder_<?php echo $data['id']; ?> = $('#hw-gallery-<?php echo $data['id']; ?>').find(".hw-gallery-preload"),
                    hw_gallery_throttle_<?php echo $data['id']; ?> = <?php echo apply_filters( 'hw_gallery_isotope_throttle', 500, $data ); ?>;

                function hw_galleryOnFinished<?php echo $data['id']; ?>(){
                    hw_gallery_container_<?php echo $data['id']; ?>.isotope('reLayout');
                    hw_gallery_container_<?php echo $data['id']; ?>.parent().css('background-image', 'none');
                }

                hw_gallery_container_<?php echo $data['id']; ?>.isotope({
                    <?php do_action( 'hw_gallery_api_hwtope_config', $data ); ?>
                    transformsEnabled: false,
                    masonry: {
                        gutterWidth: <?php echo absint( $this->get_config( 'gutter', $data ) ); ?>,
                        columnWidth: hw_galleryGetColWidth(hw_gallery_container_<?php echo $data['id']; ?>, <?php echo absint( $this->get_config( 'gutter', $data ) ); ?>)
                    },
                    onLayout: function( $elems, instance ) {
                        hw_gallery_container_<?php echo $data['id']; ?>.css('overflow', 'visible');
                        <?php do_action( 'hw_gallery_api_hwtope_layout', $data ); ?>
                    }
                });

                var hw_galleryApplyIsotope<?php echo $data['id']; ?> = hw_galleryThrottle(function(){
                    hw_gallery_container_<?php echo $data['id']; ?>.isotope('reLayout');
                }, hw_gallery_throttle_<?php echo $data['id']; ?>);

                if ( 0 !== hw_gallery_holder_<?php echo $data['id']; ?>.length ) {
                    var hw_gallery_src_attr = 'data-hw-gallery-src';
                    $.each(hw_gallery_holder_<?php echo $data['id']; ?>, function(i, el){
                        var hw_gallery_src = $(this).attr(hw_gallery_src_attr);
                        if ( typeof hw_gallery_src === 'undefined' || false === hw_gallery_src ) {
                            return;
                        }

                        var hw_gallery_image = new Image();
                        hw_gallery_image.src = hw_gallery_src;
                        $(this).attr('src', hw_gallery_src).removeAttr(hw_gallery_src_attr).css('opacity', '1');
                        hw_gallery_image.onload = function(){
                            hw_galleryApplyIsotope<?php echo $data['id']; ?>();
                        };

                        // If loading in the last image, don't throttle the reLayout method - just do it.
                        if ( (i + 1) === hw_gallery_holder_<?php echo $data['id']; ?>.length ) {
                            hw_gallery_container_<?php echo $data['id']; ?>.parent().css('background-image', 'none');
                        }
                    });
                }

                <?php do_action( 'hw_gallery_api_preload', $data ); ?>

                hw_gallerySetWidths(hw_gallery_container_<?php echo $data['id']; ?>, <?php echo absint( $this->get_config( 'gutter', $data ) ); ?>);

                $(window).smartresize(function(){
                    hw_gallerySetWidths(hw_gallery_container_<?php echo $data['id']; ?>, <?php echo absint( $this->get_config( 'gutter', $data ) ); ?>);
                    hw_gallery_container_<?php echo $data['id']; ?>.isotope({
                        <?php do_action( 'hw_gallery_api_isotope_config', $data ); // Deprecated. ?>
                        <?php do_action( 'hw_gallery_api_hwtope_config', $data ); ?>
                        transformsEnabled: false,
                        masonry: {
                            gutterWidth: <?php echo absint( $this->get_config( 'gutter', $data ) ); ?>,
                        ...
                        onLayout: function( $elems, instance ) {
                            hw_gallery_container_<?php echo $data['id']; ?>.css('overflow', 'visible');
                            <?php do_action( 'hw_gallery_api_isotope_layout', $data ); // Deprecated. ?>
                            <?php do_action( 'hw_gallery_api_hwtope_layout', $data ); ?>
                        }
                    }, hw_galleryOnFinished<?php echo $data['id']; ?>);
                });

                <?php do_action( 'hw_gallery_api_isotope', $data ); // Deprecated. ?>
                <?php do_action( 'hw_gallery_api_hwtope', $data ); ?>

                <?php
                // Fancybox: Start
                if ( $this->get_config( 'lightbox_enabled', $data ) ) {
                    ?>
                    $('.hw-gallery-<?php echo $data['id']; ?>').hw_gallerybox({
                        <?php do_action( 'hw_gallery_api_config', $data ); ?>
                        cyclic: true,
                        centerOnScroll: true,    
                        tpl: {
                            error    : '<p class="hw-gallerybox-error">The requested content cannot be loaded.<br/>Please try again later.</p>',
                            closeBtn : '<a title="<?php _e( 'Close', 'hw-gallery' ); ?>" class="hw-gallerybox-item hw-gallerybox-close" href="javascript:;"></a>',
                            next     : '<a title="<?php _e( 'Next', 'hw-gallery' ); ?>" class="hW-gallerybox-nav hw-gallerybox-next" href="javascript:;"><span></span></a>',
                            prev     : '<a title="<?php _e( 'Previous', 'hw-gallery' ); ?>" class="hw-gallerybox-nav hw-gallerybox-prev" href="javascript:;"><span></span></a>'
                        },
                        <?php do_action( 'hw_gallery_api_config_callback', $data ); ?>
                        onStart: function(data, index, opts){
                            $(window).on({
                                'resize' : function(){
                                    $.hw_gallerybox.resize();
                                    $.hw_gallerybox.center();
                                }
                            });
                        ...
                        afterShow: function() {
                            ...
                            if ( $this->get_config( 'title_display', $data ) == 'float_wrap' ) {
                                ?>
                                if ( ! $( 'div.hw-gallerybox-title' ).hasClass( 'hw-gallerybox-title-text-wrap' ) ) {
                                    $( 'div.hw-gallerybox-title' ).addClass( 'hw-gallerybox-title-text-wrap' );
                                }
                                <?php
                            }
                            ?>
                    ...
                    <?php 
                    do_action( 'hw_gallery_api_lightbox', $data );
                } // Fancybox: End
                ?>

            <?php do_action( 'hw_gallery_api_end', $data ); endforeach;

            // Minify before outputting to improve page load time.
            do_action( 'hw_gallery_api_end_global' );
    ...
    public function get_gallery_classes( $data ) {
        ...
        $classes[] = 'hw-gallery-lightbox-theme-' . $this->get_config( 'lightbox_theme', $data );
    ...
    public function get_image_src( $id, $item, $data, $mobile = false ) {
        ...
        if ( isset( $data['config'][$type] ) && $data['config'][$type] ) {
            ...
            if ( is_wp_error( $cropped_image ) ) {
                ...
                return apply_filters( 'hw_gallery_image_src', $image, $id, $item, $data );
            } else {
                return apply_filters( 'hw_gallery_image_src', $cropped_image, $id, $item, $data );
            }
        } else {
            return apply_filters( 'hw_gallery_image_src', $image, $id, $item, $data );
        }
    ...
    public function minify( $string, $stripDoubleForwardslashes = true ) {
        ...
        $stripDoubleForwardslashes = apply_filters( 'hw_gallery_minify_strip_double_forward_slashes', $stripDoubleForwardslashes );
        ...
        return apply_filters( 'hw_gallery_minified_string', $clean, $string );
     ...
     * @since 1.0.0
     *
     * @return object The HW_Gallery_Shortcode_Lite object.
     */
    public static function get_instance() {

        if ( ! isset( self::$instance ) && ! ( self::$instance instanceof HW_Gallery_Shortcode_Lite ) ) {
            self::$instance = new HW_Gallery_Shortcode_Lite();
        }
...
$hw_gallery_shortcode_lite = HW_Gallery_Shortcode_Lite::get_instance();