...
if ( !class_exists('WordpressPopularPosts') ) {
	...
	add_action( 'hw_widgets_init', 'load_wpp' );
	...
	class WordpressPopularPosts extends WP_Widget {
		...
		public function __construct() {
			...
			parent::__construct(
				'wpp',
				'HW Bài viết xem nhiều',
				array(
					'classname'		=>	'popular-posts',
					'description'	=>	__( 'Bài viết xem nhiều trên website.', $this->plugin_slug )
				)
			);
			...
			//by hoang,setup metaslider themes
			if(class_exists('HW_SKIN') /*&& !isset($this->skin)*/){
			   $this->skin = new HW_SKIN($this,plugin_dir_path(__FILE__),'hw_popular_posts_skins','popular-posts-skin.php','skins');
			   #$this->skin->skin_name='popular-posts-skin.php';
			   $this->skin->skin_folder='hw_popular_posts_skins';	//you will see js object 'hw_skin_hw_mlslider_skins'
		...
		public function widget( $args, $instance ) {
			...
			//by hoangweb
			if(isset($this->skin)){
				   //load actived skin which contains template hooks
				   $file = $this->skin->get_skin_file($instance['skin']);
				   if(file_exists($file)) {
					   include_once($file);
				   }
			 }
			...
			if ( '' != $instance['title'] ) {
				...
				if ($instance['markup']['custom_html'] && $instance['markup']['title-start'] != "" && $instance['markup']['title-end'] != "" ) {
					echo apply_filters('hwwpp_widget_before_title',htmlspecialchars_decode($instance['markup']['title-start'], ENT_QUOTES)) ; 
					echo $title ;
					echo apply_filters('hwwpp_widget_after_title', htmlspecialchars_decode($instance['markup']['title-end'], ENT_QUOTES));
					//remove all filter handlers after done
					remove_all_filters('hwwpp_widget_before_title');
					remove_all_filters('hwwpp_widget_after_title');
				} else {
		...
		public function add_plugin_settings_link( $links, $file ){
			...
			if ( is_plugin_active($this_plugin) && $file == $this_plugin ) {
				$links[] = '<a href="' . admin_url( 'options-general.php?page=wordpress-popular-posts' ) . '">Cài đặt</a>';
			}
		...
		public function check_admin_notices() {
			...
			printf(
				...
				join( '</p><p>', $errors ),
				'Bài viết phổ biến - Popular Posts'
			);
		...
		private function __get_popular_posts( $instance ) {
			...
			if ( !is_array($mostpopular) || empty($mostpopular) ) {
				return apply_filters('hwwpp_no_posts', "<p>".__('Sorry. No data so far.', $this->plugin_slug)."</p>");
			}
		...
		private function __render_popular_post($p, $instance) {
			...
			$excerpt = $this->_get_excerpt($p, $instance);	//summary
			//views
			...
			$num_comments = number_format_i18n( $comments );
			
			$rating = $this->_get_rating($p, $instance);
			//{date}
			$date = $this->_get_date($p, $instance);

			$_stats = join(' | ', $this->_get_stats($p, $instance));	//stats
				...
				$data = array(
					...
					'target' => $this->user_settings['tools']['link']['target'],
					'rating' => $rating
				);
				//$instance['markup']['post-html']
				if($instance['markup']['custom_html']){
					$content = $this->__format_content( htmlspecialchars_decode(apply_filters('hwwpp_loop_custom_html',0), ENT_QUOTES ), $data, $instance['rating'] ). "\n";
				    remove_all_filters('hwwpp_loop_custom_html');   //should remove after calling
				}
				else{
					$obj = $this;
					$content = apply_filters('hwwpp_loop_post',compact('permalink','title','title_sub','author','post_cat','thumb','excerpt','pageviews','views','comments','num_comments','rating','date','_stats','instance','p','obj')). "\n";
					remove_all_filters('hwwpp_loop_post');   //should remove after calling
				}
		...
		*/
		...
		private function __format_content($string, $data = array(), $rating) {
			...
			$params = array();
			$pattern = '/\{(excerpt|summary|stats|title|image|thumb|thumb_img|rating|score|url|text_title|author|category|views|comments|date|id|title_sub|target|rating_live)\}/i';
			...
			//by hoangweb custom format content
			if( in_array("{id}", $matches[0]) ){
				$string = str_replace( "{id}", $data['id'], $string );
			}
			if( in_array("{title_sub}", $matches[0]) ){
				$string = str_replace( "{title_sub}", $data['title_sub'], $string );
			}
			if( in_array("{target}", $matches[0]) ){
				$string = str_replace( "{target}", $data['target'], $string );
			}
			if( in_array("{rating_live}", $matches[0]) ){
				$string = str_replace( "{rating_live}", $data['rating'], $string );
			}
