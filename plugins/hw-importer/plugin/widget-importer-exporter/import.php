...
class HW_Widget_Import {
     ...
     * @var
     */
    protected static $hw_wie_import_results;

    /**
     * main class constructor
     */
    public function __construct() {
        //add_action( 'load-tools_page_widget-importer-exporter', array(&$this, '_hw_wie_upload_import_file') );
    }

    /**
     * @return HW_Widget_Import
     */
     ...
     * @since 0.3
     */
    function _hw_wie_upload_import_file() {
        ...
        if ( ! empty( $_POST ) && ! empty( $_FILES['wie_import_file'] ) && check_admin_referer( 'wie_import', 'wie_import_nonce' ) ) { // check_admin_referer prints fail page and dies
            ...
            // Process import file
            $this->hw_wie_process_import_file( $file_data['file'] );
     ...
     */
    function hw_wie_process_import_file( $file ) {
        ...
        $this->hw_wie_import_data( $data );

    }

    /**
     * process import widgets data
     * @param $data
     */
    function hw_wie_process_import($data) {#__print($data);#exit;
        // Import the widget data
        // Make results available for display on import/export page
        if(!empty($data) && (is_object($data) || is_array($data))) {
            self::$hw_wie_import_results = $this->hw_wie_import_data((object) $data );
        }
    }
    /**
     * Show import results
     ...
     * @since 0.3
     */
    function hw_wie_show_import_results() {
        if (! $this->have_import_results() ) {
            return ;
        }
        printf(
            __( 'You can manage your <a href="%s">Widgets</a> or <a href="%s">Go Back</a>.', 'widget-importer-exporter' ),
            admin_url( 'widgets.php' ),
            admin_url( basename( $_SERVER['PHP_SELF'] ) . '?page=' . (isset($_GET['page'])? $_GET['page']:1) )
        );
        ?>
        <table id="wie-import-results">
            ...
            <?php
            // Loop sidebars
            $results = self::$hw_wie_import_results;
     ...
     */
    public function have_import_results() {

        if ( ! empty( self::$hw_wie_import_results ) ) {
            return true;
        }
    ...
    function hw_wie_import_data( $data ) {
        ...
        foreach ( $data as $sidebar_id => $widgets ) {
            ...
            foreach ( $widgets as $widget_instance_id => $widget ) {
                ...
                if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
                    ...
                    foreach ( $single_widget_instances as $check_id => $check_widget ) {
                        ...
                        if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget['instance'] == $check_widget ) {
                            ...
                            if($widget['args']['update']) $delete_old[] = array_search("$id_base-$check_id", $sidebar_widgets);
                            else $fail = true;  //not allow to override widget
                ...
                $widget = apply_filters( 'hw_wie_widget_settings', $widget['instance'] ); // object
                ...
                else $title = __( 'No Title', 'widget-importer-exporter' );

                $results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $title; // show "No Title" if widget instance is untitled
