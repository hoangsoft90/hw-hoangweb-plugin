 ...
 */
function hw_wie_generate_export_data() {
	...
	$data = apply_filters( 'hw_wie_unencoded_export_data', $sidebars_widget_instances );

	// Encode the data for file contents
	$encoded_data = json_encode( $data );

	// Return contents
	return apply_filters( 'hw_wie_generate_export_data', $encoded_data );
 ...
 * @since 0.1
 */
function hw_wie_send_export_file() {
	...
	if ( ! empty( $_GET['export'] ) ) {
		...
		$filename = apply_filters( 'hw_wie_export_filename', $filename );
