...
class HW_MetaSlide {
    ...
    public function set_slider( $id ) {
        ...
        $this->settings = get_post_meta( $id, 'hw-ml-slider_settings', true );
    }
    ...
    public function ajax_change_slide_image() {
        ...
        if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'hw_metaslider_changeslide' ) ) {
    ...
    public function get_slide_html() {
        ...
        if ( is_admin() && isset( $_GET['page'] ) && $_GET['page'] == 'hw-metaslider-theme-editor' ) {
    ...
    public function slide_is_unassigned_or_image_slide( $slider_id, $slide_id ) {
        ...
        $type = get_post_meta( $slide_id, 'hw-ml-slider_type', true );
    ...
    public function tag_slide_to_slider() {
        ...
        if ( ! term_exists( $this->slider->ID, 'hw-ml-slider' ) ) {
            // create the taxonomy term, the term is the ID of the slider itself
            wp_insert_term( $this->slider->ID, 'hw-ml-slider' );
        }

        // get the term thats name is the same as the ID of the slider
        $term = get_term_by( 'name', $this->slider->ID, 'hw-ml-slider' );
        // tag this slide to the taxonomy term
        wp_set_post_terms( $this->slide->ID, $term->term_id, 'hw-ml-slider', true );
    ...
    public function get_delete_button_html() {
        ...
        return "<a title='" . __("Delete slide", "hw-metaslider") . "' class='tipsy-tooltip-top delete-slide dashicons dashicons-trash' href='?page=hw-metaslider&amp;id={$this->slider->ID}&amp;deleteSlide={$this->slide->ID}'>" . __("Delete slide", "hw-metaslider") . "</a>";
    ...
    public function get_change_image_button_html() {
        ...
        return apply_filters("hw_metaslider_change_image_button_html", "", $this->slide);
