 ...
 */
class HW_MetaImageSlide extends HW_MetaSlide {
    ...
    public function __construct() {
        ...
        add_filter( 'hw_metaslider_get_image_slide', array( $this, 'get_slide' ), 10, 2 );
        add_action( 'hw_metaslider_save_image_slide', array( $this, 'save_slide' ), 5, 3 );
    ...
    public function ajax_create_slide() {
        // security check
        if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'hw_metaslider_addslide' ) ) {
            echo "<tr><td colspan='2'>" . __( "Security check failed. Refresh page and try again.", 'hw-metaslider' ) . "</td></tr>";
            wp_die();
        }
        ...
        if ( is_array( $selection ) && count( $selection ) && $slider_id > 0 ) {
            ...
            foreach ( $selection as $slide_id ) {
                ...
                if ( $this->slide_exists_in_slideshow( $slider_id, $slide_id ) ) {
                    ...
                    echo "<tr><td colspan='2'>ID: {$slide_id} \"" . get_the_title( $slide_id ) . "\" - " . __( "Failed to add slide. Slide already exists in slideshow.", 'hw-metaslider' ) . "</td></tr>";
                
                } else if ( !$this->slide_is_unassigned_or_image_slide( $slider_id, $slide_id ) ) {
                    
                    echo "<tr><td colspan='2'>ID: {$slide_id} \"" . get_the_title( $slide_id ) . "\" - " . __( "Failed to add slide. Slide is not of type 'image'.", 'hw-metaslider' ) . "</td></tr>";
    ...
    public function ajax_resize_slide() {
        ...
        check_admin_referer( 'hw_metaslider_resize' );
        ...
        $settings = get_post_meta( $slider_id, 'hw-ml-slider_settings', true );

        // create a copy of the correct sized image
        $imageHelper = new HW_MetaSliderImageHelper(
            $slide_id,
            $settings['width'],
            $settings['height'],
        ...
        do_action( "hw_metaslider_ajax_resize_image_slide", $slide_id, $slider_id, $settings );
    ...
    protected function get_admin_slide() {
        ...
        $imageHelper = new HW_MetaSliderImageHelper( $this->slide->ID, 150, 150, 'false', $this->use_wp_image_editor() );
        $thumb       = $imageHelper->get_image_url();
        $slide_label = apply_filters( "hw_metaslider_image_slide_label", __( "Image Slide", "hw-metaslider" ), $this->slide, $this->settings );
    ...
    public function get_admin_tabs() {
        ...
        $url = esc_attr( get_post_meta( $slide_id, 'hw-ml-slider_url', true ) );
        $title = esc_attr( get_post_meta( $slide_id, 'hw-ml-slider_title', true ) );
        $target = get_post_meta( $slide_id, 'hw-ml-slider_new_window', true ) ? 'checked=checked' : '';
        $caption = esc_textarea( $this->slide->post_excerpt );

        $general_tab = "<textarea name='attachment[{$slide_id}][post_excerpt]' placeholder='" . __( "Caption", "hw-metaslider" ) . "'>{$caption}</textarea>
                        <input class='url' type='text' name='attachment[{$slide_id}][url]' placeholder='" . __( "URL", "hw-metaslider" ) . "' value='{$url}' />
                        <div class='new_window'>
                        <label>" . __( "New Window", "hw-metaslider" ) . "<input type='checkbox' name='attachment[{$slide_id}][new_window]' {$target} /></label>
                        </div>";

        if ( ! $this->is_valid_image() ) {
            $message = __( "Warning: Image data does not exist. Please re-upload the image.", "hw-metaslider" );

            $general_tab = "<div class='warning'>{$message}</div>" . $general_tab;
        }

        $seo_tab = "<div class='row'><label>" . __( "Image Title Text", "hw-metaslider" ) . "</label></div>
                    <div class='row'><input type='text' size='50' name='attachment[{$slide_id}][title]' value='{$title}' /></div>
                    <div class='row'><label>" . __( "Image Alt Text", "hw-metaslider" ) . "</label></div>
                    <div class='row'><input type='text' size='50' name='attachment[{$slide_id}][alt]' value='{$alt}' /></div>";

        $tabs = array(
            'general' => array(
                'title' => __( "General", "hw-metaslider" ),
                'content' => $general_tab
            ),
            'seo' => array(
                'title' => __( "SEO", "hw-metaslider" ),
                'content' => $seo_tab
            )
        );
        ...
        if ( version_compare( get_bloginfo('version'), 3.9, '>=' ) ) {
            ...
            $crop_position = get_post_meta( $slide_id, 'hw-ml-slider_crop_position', true);

            if ( ! $crop_position ) {
                $crop_position = 'center-center';
            }
        
            $crop_tab = "<div class='row'><label>" . __( "Crop Position", "hw-metaslider" ) . "</label></div>
                        <div class='row'>
                            <select class='crop_position' name='attachment[{$slide_id}][crop_position]'>
                                <option value='left-top' " . selected( $crop_position, 'left-top', false ) . ">" . __( "Top Left", "hw-metaslider" ) . "</option>
                                <option value='center-top' " . selected( $crop_position, 'center-top', false ) . ">" . __( "Top Center", "hw-metaslider" ) . "</option>
                                <option value='right-top' " . selected( $crop_position, 'right-top', false ) . ">" . __( "Top Right", "hw-metaslider" ) . "</option>
                                <option value='left-center' " . selected( $crop_position, 'left-center', false ) . ">" . __( "Center Left", "hw-metaslider" ) . "</option>
                                <option value='center-center' " . selected( $crop_position, 'center-center', false ) . ">" . __( "Center Center", "hw-metaslider" ) . "</option>
                                <option value='right-center' " . selected( $crop_position, 'right-center', false ) . ">" . __( "Center Right", "hw-metaslider" ) . "</option>
                                <option value='left-bottom' " . selected( $crop_position, 'left-bottom', false ) . ">" . __( "Bottom Left", "hw-metaslider" ) . "</option>
                                <option value='center-bottom' " . selected( $crop_position, 'center-bottom', false ) . ">" . __( "Bottom Center", "hw-metaslider" ) . "</option>
                                <option value='right-bottom' " . selected( $crop_position, 'right-bottom', false ) . ">" . __( "Bottom Right", "hw-metaslider" ) . "</option>
                            </select>
                        </div>";

            $tabs['crop'] = array(
                'title' => __( "Crop", "hw-metaslider" ),
                'content' => $crop_tab
            );
            
        }

        return apply_filters("hw_metaslider_image_slide_tabs", $tabs, $this->slide, $this->slider, $this->settings);
    ...
    public function is_valid_image() {
        ...
        return apply_filters( 'hw_metaslider_is_valid_image', $is_valid, $this->slide );
    }
    ...
    public function use_wp_image_editor() {
        ...
        return apply_filters( 'hw_metaslider_use_image_editor', $this->is_valid_image(), $this->slide );
    ...
    protected function get_public_slide() {
        ...
        $slide = array(
            'id' => $this->slide->ID,
            'url' => __( get_post_meta( $this->slide->ID, 'hw-ml-slider_url', true ) ),
            'title' => __( get_post_meta( $this->slide->ID, 'hw-ml-slider_title', true ) ),
            'target' => get_post_meta( $this->slide->ID, 'hw-ml-slider_new_window', true ) ? '_blank' : '_self',
            'src' => $thumb,
    ...
    private function get_nivo_slider_markup( $slide ) {
        ...
        $attributes = apply_filters( 'hw_metaslider_nivo_slider_image_attributes', array(
                'src' => $slide['src'],
                'height' => $slide['height'],
                'width' => $slide['width'],
        ...
        $anchor_attributes = apply_filters( 'hw_metaslider_nivo_slider_anchor_attributes', array(
                'href' => $slide['url'],
                'target' => $slide['target']
            ), $slide, $this->slider->ID );
        ...
        return apply_filters( 'hw_metaslider_image_nivo_slider_markup', $html, $slide, $this->settings );
    ...
    private function get_flex_slider_markup( $slide ) {
        ...
        $attributes = apply_filters( 'hw_metaslider_flex_slider_image_attributes', $image_attributes, $slide, $this->slider->ID );

        $html = $this->build_image_tag( $attributes );

        $anchor_attributes = apply_filters( 'hw_metaslider_flex_slider_anchor_attributes', array(
                'href' => $slide['url'],
                'target' => $slide['target']
            ), $slide, $this->slider->ID );
        ...
        $attributes = apply_filters( 'hw_metaslider_flex_slider_list_item_attributes', array(
        ...
        return apply_filters( 'hw_metaslider_image_flex_slider_markup', $html, $slide, $this->settings );
    ...
    private function get_coin_slider_markup( $slide ) {
        ...
        $attributes = apply_filters( 'hw_metaslider_coin_slider_image_attributes', array(
                'src' => $slide['src'],
                'height' => $slide['height'],
                'width' => $slide['width'],
                'alt' => $slide['alt'],
        ...
        $attributes = apply_filters( 'hw_metaslider_coin_slider_anchor_attributes', array(
                'href' => strlen( $slide['url'] ) ? $slide['url'] : 'javascript:void(0)'
            ), $slide, $this->slider->ID );

        $html = $this->build_anchor_tag( $attributes, $html );

        return apply_filters( 'hw_metaslider_image_coin_slider_markup', $html, $slide, $this->settings );
    ...
    private function get_responsive_slides_markup( $slide ) {
        ...
        $attributes = apply_filters( 'hw_metaslider_responsive_slider_image_attributes', array(
                'src' => $slide['src'],
                'height' => $slide['height'],
                'width' => $slide['width'],
                'alt' => $slide['alt'],
        ...
        $anchor_attributes = apply_filters( 'hw_metaslider_responsive_slider_anchor_attributes', array(
                'href' => $slide['url'],
                'target' => $slide['target']
            ), $slide, $this->slider->ID );
        ...
        return apply_filters( 'hw_metaslider_image_responsive_slider_markup', $html, $slide, $this->settings );
