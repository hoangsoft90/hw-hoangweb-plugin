...
class HW_MetaSlider {
    ...
    public function __construct( $id, $shortcode_settings ) {
        ...
        $this->identifier = 'hw_metaslider_' . $this->id;
        $this->save();
    ...
    private function get_settings() {
        $settings = get_post_meta( $this->id, 'hw-ml-slider_settings', true );
    ...
    public function get_default_parameters() {
        ...
        $params = apply_filters( 'hw_metaslider_default_parameters', $params );
    ...
    private function save() {
        ...
        if ( isset( $_POST['settings'] ) ) {
            check_admin_referer( 'hw_metaslider_save_' . $this->id );
    ...
    public function get_slides() {
        ...
        $args = apply_filters( 'hw_metaslider_populate_slides_args', $args, $this->id, $this->settings );
    ...
    private function populate_slides() {
        ...
        while ( $query->have_posts() ) {
            ...
            $type = get_post_meta( $query->post->ID, 'hw-ml-slider_type', true );
            $type = $type ? $type : 'image'; // backwards compatibility, fall back to 'image'

            if ( has_filter( "hw_metaslider_get_{$type}_slide" ) ) {
                $return = apply_filters( "hw_metaslider_get_{$type}_slide", $query->post->ID, $this->id );
    ...
    public function render_public_slides() {
        ...
        $slideshow = apply_filters( 'hw_metaslider_slideshow_output', $slideshow, $this->id, $this->settings );
    ...
    private function get_container_id() {
        $container_id = 'hw_metaslider_container_' . $this->id;

        $id = apply_filters( 'hw_metaslider_container_id', $container_id, $this->id, $this->settings );
    ...
    private function get_container_class() {
        $class = "hw-metaslider metaslider-{$this->get_setting( 'type' )} metaslider-{$this->id} ml-slider";
        ...
        $class = apply_filters( 'hw_metaslider_css_classes', $class, $this->id, $this->settings );
    ...
    private function get_container_style() {
        ...
        $style = apply_filters( 'hw_metaslider_container_style', $style, $this->id, $this->settings );
    ...
    private function get_inline_javascript() {
        ...
        $script .= "\n            jQuery('#" . $identifier . "')." . $this->js_function . "({ ";
    ...
    private function get_html_after() {
        ...
        $html = apply_filters( "hw_metaslider_{$type}_slider_html_after", "", $this->id, $this->settings );
    ...
    private function get_custom_javascript_before() {
        ...
        if ( $this->get_setting( 'noConflict' ) == 'true' && $type == 'flex' ) {
            $javascript = "jQuery('#metaslider_{$this->id}').addClass('flexslider'); // theme/plugin conflict avoidance";
        }

        $custom_js = apply_filters( "hw_metaslider_{$type}_slider_javascript_before", $javascript, $this->id );
    ...
    private function get_custom_javascript_after() {
        ...
        $custom_js = apply_filters( "hw_metaslider_{$type}_slider_javascript", "", $this->id );
    ...
    private function get_javascript_parameters() {
        ...
        $type = $this->get_setting( 'type' );
        $options = apply_filters( "hw_metaslider_{$type}_slider_parameters", $options, $this->id, $this->settings );
    ...
    private function get_inline_css() {
        $css = apply_filters( "hw_metaslider_css", "", $this->settings, $this->id );

        // use this to add the scoped attribute for HTML5 validation (if needed)
        $attributes = apply_filters( "hw_metaslider_style_attributes", "", $this->settings, $this->id );

        if ( strlen( $css ) ) {
            return "<style type=\"text/css\"{$attributes} id=\"hw-metaslider-css-{$this->id}\">{$css}\n    </style>";
        }
    ...
    public function enqueue_scripts() {
        if ( $this->get_setting( 'printJs' ) == 'true' ) {
            wp_enqueue_script( 'hw-metaslider-' . $this->get_setting( 'type' ) . '-slider', HW_METASLIDER_ASSETS_URL . $this->js_path, array( 'jquery' ), HW_METASLIDER_VERSION );
        }
        ...
        if ( $this->get_setting( 'printCss' ) == 'true' ) {
            ...
            wp_enqueue_style( 'hw-metaslider-' . $this->get_setting( 'type' ) . '-slider', HW_METASLIDER_ASSETS_URL . $this->css_path, false, HW_METASLIDER_VERSION );
            wp_enqueue_style( 'hw-metaslider-public', HW_METASLIDER_ASSETS_URL . 'metaslider/public.css', false, HW_METASLIDER_VERSION );
        }

        do_action( 'hw_metaslider_register_public_styles' );
    }
    ...
    public function update_settings( $new_settings ) {
        ...
        $checkboxes = apply_filters( "hw_metaslider_checkbox_settings", array( 'noConflict', 'fullWidth', 'hoverPause', 'links', 'reverse', 'random', 'printCss', 'printJs', 'smoothHeight', 'center', 'carouselMode', 'autoPlay' ) );
        ...
        update_post_meta( $this->id, 'hw-ml-slider_settings', array_merge( (array)$old_settings, $new_settings ) );
    ...
    private function delete_slide( $slide_id ) {
        ...
        $new_terms = array();
        $current_terms = wp_get_object_terms( $slide_id, 'hw-ml-slider', array( 'fields' => 'ids' ) );
        $term = get_term_by( 'name', $this->id, 'hw-ml-slider' );
        ...
        return wp_set_object_terms( $slide_id, $new_terms, 'hw-ml-slider' );
    }
    ...
    private function update_slides( $data ) {
        foreach ( $data as $slide_id => $fields ) {
            do_action( "hw_metaslider_save_{$fields['type']}_slide", $slide_id, $this->id, $fields );
        }
    }
}